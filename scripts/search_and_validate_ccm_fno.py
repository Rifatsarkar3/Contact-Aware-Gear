"""Leakage-safe architecture search + rigorous, capacity-controlled validation for CCM-FNO.

Learned from the DCT-FNO study (`docs/DCT_FNO_STUDY_2026-07-17.md`): a
search that is free to change model capacity can produce a false-positive
win that is really just "more parameters," not the mechanism under test.
This script avoids that failure mode from the start in two ways:

1. The architecture search holds the *global* branch fixed at vanilla FNO's
   own default width/modes/layers throughout, varying only the local-branch
   hyperparameters (window size, local channel width) -- so any win found
   during search is attributable to the local mechanism, not to secretly
   growing the global backbone.
2. The final validation still measures the winning configuration's total
   parameter count against vanilla FNO's default, and trains a
   capacity-matched vanilla FNO control (extra width, no local branch) for
   a fair, unconfounded final comparison -- computed and reported alongside
   the raw comparison, not only after a first result looks like a win.
"""
...
def run_name_for(window: float, width: int, seed: int, epochs: int) -> str:
    ...
    return f"ccm_fno_lw{width}_lwin{window:g}_lsz{LOCAL_SIZE}{suffix}"
...
def screen() -> tuple[float, int]:
    ...
    out_path = ROOT / "outputs" / "smoke" / DATA.stem / "ccm_fno_architecture_search.json"
...
def validate(window: float, width: int) -> dict:
    ...
    out_path = ROOT / "outputs" / "smoke" / DATA.stem / "ccm_fno_validated_summary.json"
    out_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

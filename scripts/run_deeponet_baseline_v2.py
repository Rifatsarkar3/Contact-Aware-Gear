"""Replicate DeepONet's baseline comparison on v2 (independently sampled, 2.5x scale).

Same two configurations as `run_deeponet_baseline_v1.py` (read that
docstring first): the leakage-safely-searched winner (width=32, basis_dim=32,
70,371 params) and a capacity-matched control (width=98, basis_dim=128,
663,093 params). Hyperparameters are reused from v1's search, not
re-searched on v2 -- this project's established convention for every other
mechanism (`docs/AL_MISRANKING_COMBINED_STUDY_2026-08-04.md` and others).
"""

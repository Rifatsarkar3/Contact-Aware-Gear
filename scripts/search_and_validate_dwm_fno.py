"""Leakage-safe architecture search + rigorous validation for DWM-FNO.

Same protocol as `search_and_validate_ccm_fno.py`/`search_and_validate_hgm_fno.py`.
DWM-FNO combines both previously-null CCM-FNO and HGM-FNO local branches
(contact-centered and hotspot-guided) in one model, on the hypothesis that
they attend to two physically distinct stress-concentration locations that
might be complementary even though neither helped alone.
"""
...
def run_name_for(window: float, width: int, seed: int, epochs: int) -> str:
    ...
    return f"dwm_fno_lw{width}_lwin{window:g}_lsz{LOCAL_SIZE}{suffix}"
...
def screen() -> tuple[float, int]:
    ...
    out_path = ROOT / "outputs" / "smoke" / DATA.stem / "dwm_fno_architecture_search.json"
...
def validate(window: float, width: int) -> dict:
    ...
    out_path = ROOT / "outputs" / "smoke" / DATA.stem / "dwm_fno_validated_summary.json"
    out_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
